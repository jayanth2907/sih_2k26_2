      ...
      <div className="space-y-3 pb-24 max-w-lg mx-auto select-none">
        ...
        <MobileCard className="p-3 bg-slate-900 border-slate-800 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            ...
            {selectedIncident.status === 'OPEN' && (
              <div className="col-span-2">
                <TouchButton
                  ...
                  fullWidth
                  size="md"
                  onClick={() => handleTransition('TRIAGED', "Incident triaged and categorized for field investigation.")}
                  icon={<CheckSquare className="w-4 h-4 text-amber-950" />}
                >
                  {t('incidentTriageBtn')}
                </TouchButton>
              </div>
            ...
            {selectedIncident.status === 'TRIAGED' && (
              <div className="col-span-2">
                <TouchButton
                  ...
                  fullWidth
                  size="md"
                  onClick={() => handleTransition('ASSIGNED', "Incident assigned to field response team for on-site investigation.", undefined, user?.id)}
                  icon={<UserCheck className="w-4 h-4 text-amber-950" />}
                >
                  {t('incidentAssignBtn')}
                </TouchButton>
              </div>
            ...
            {selectedIncident.status === 'ASSIGNED' && (
              <div className="col-span-2">
                <TouchButton
                  ...
                  icon={<Flame className="w-4 h-4 text-amber-950" />}
                >
            ...
            {/* ESCALATED -> Resume Response / Mitigate */}
            {selectedIncident.status === 'ESCALATED' && (
              <>
                <TouchButton
                  ...
                  fullWidth
                  size="md"
                  onClick={() => handleTransition('IN_PROGRESS', "Response operations resumed following escalation review.")}
                ...
                <TouchButton
                  ...
                  fullWidth
                  size="md"
                  onClick={() => handleTransition('RESOLVED', "Escalated incident mitigated under emergency protocol.", "Emergency stabilization completed.")}
            ...
            {/* RESOLVED (Non-supervisor view) */}
            {selectedIncident.status === 'RESOLVED' && !isSupervisor && (
              <div className="col-span-2 p-2 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-300 text-xs font-mono flex items-center gap-2">
                <Clock className="w-4 h-4 text-blue-400 flex-shrink-0" />
                <span>Pending supervisor verification sign-off</span>
              </div>
            )}
            ...
            {/* VERIFIED (Non-supervisor view) */}
            {selectedIncident.status === 'VERIFIED' && !isSupervisor && (
              <div className="col-span-2 p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-mono flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                <span>Verified by supervisor — pending administrative closure</span>
              </div>
            )}

            {/* CLOSED -> Terminal State */}
            {selectedIncident.status === 'CLOSED' && (
              <div className="col-span-2 p-2 rounded-xl bg-slate-950 border border-slate-800 text-slate-400 text-xs font-mono flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0" />
                <span>Incident closed and archived into immutable audit ledger.</span>
              </div>
            )}
          </div>
        </MobileCard>

...
export const MobileFieldReportingScreen: React.FC<MobileFieldReportingScreenProps> = ({
  ...
  const [prodEvidence, setProdEvidence] = useState<{ name?: string; url?: string; hash?: string; size?: number; type?: string }>({});
  ...
  const [envEvidence, setEnvEvidence] = useState<{ name?: string; url?: string; hash?: string; size?: number; type?: string }>({});
  ...
  const [compEvidence, setCompEvidence] = useState<{ name?: string; url?: string; hash?: string; size?: number; type?: string }>({});
  ...
  // Camera / File Evidence Upload Handler with SHA-256 calculation & validation
  const handleEvidenceUpload = async (
    e: React.ChangeEvent<HTMLInputElement>,
    target: 'PROD' | 'ENV' | 'COMP'
  ) => {
    ...
    if (file.size > MAX_SIZE) {
      setNotice({
        type: 'ERROR',
        message: 'Evidence file exceeds maximum permitted size of 15MB.'
      });
      e.target.value = '';
      return;
    }
    ...
    if (!validMimes.includes(file.type) && !file.type.startsWith('image/')) {
      setNotice({
        type: 'ERROR',
        message: 'Invalid file format. Please upload an image (JPEG, PNG, WebP) or PDF document.'
      });
      e.target.value = '';
      return;
    }
          ...
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-t-3xl sm:rounded-2xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="space-y-1">
                <label className="text-slate-400 block font-mono text-[11px]">Location Context</label>
                <div className="flex items-center justify-between p-2 rounded-xl bg-slate-950 border border-slate-800 text-xs">
                  <div className="flex items-center gap-1.5 text-slate-300 truncate">
                    ...
                    <span className="truncate font-mono text-[11px]">
                      {prodGps.lat && prodGps.lon ? `${prodGps.lat}, ${prodGps.lon}` : 'No GPS coordinate fixed'}
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => handleCaptureGps('PROD')}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold bg-slate-800 text-amber-400 border border-slate-700 hover:bg-slate-700"
                  >
                    Fix GPS
                  </button>
                </div>
              ...
              <div className="space-y-1.5">
                <label className="text-slate-400 block font-mono text-[11px] flex items-center justify-between">
                  <span>{t('evidenceAttachmentTitle')}</span>
                  <span className="text-[10px] text-slate-500 font-mono">SHA-256 Fingerprint</span>
                </label>

                {!prodEvidence.name ? (
                  <label className="w-full p-2.5 bg-slate-950 border border-slate-800 hover:border-slate-700 rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all">
                    <Camera className="w-4 h-4 text-amber-400" />
                    <span className="text-xs text-slate-300 font-medium">Capture / Upload Evidence</span>
                    <input
                      type="file"
                      accept="image/*,application/pdf"
                      onChange={e => handleEvidenceUpload(e, 'PROD')}
                      className="hidden"
                    />
                  </label>
                ) : (
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        {prodEvidence.url && prodEvidence.type?.startsWith('image/') ? (
                          <img
                            src={prodEvidence.url}
                            alt="Evidence preview"
                            className="w-10 h-10 object-cover rounded-lg border border-slate-700 shrink-0"
                          />
                        ) : (
                          <div className="w-10 h-10 bg-slate-900 border border-slate-700 rounded-lg flex items-center justify-center shrink-0">
                            <FileText className="w-5 h-5 text-amber-400" />
                          </div>
                        )}
                        <div className="min-w-0">
                          <p className="text-xs text-slate-200 font-medium truncate">{prodEvidence.name}</p>
                          {prodEvidence.size && (
                            <p className="text-[10px] text-slate-500 font-mono">
                              {(prodEvidence.size / 1024).toFixed(1)} KB
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <label className="px-2 py-1 rounded-lg text-[10px] font-mono font-bold bg-slate-800 text-slate-300 hover:text-white border border-slate-700 cursor-pointer">
                          <span>Replace</span>
                          <input
                            type="file"
                            accept="image/*,application/pdf"
                        ...
                        <button
                          type="button"
                          onClick={() => setProdEvidence({})}
                          className="px-2 py-1 rounded-lg text-[10px] font-mono font-bold bg-red-950/40 text-red-400 hover:text-red-300 border border-red-800/50"
                        >
                          Remove
                        </button>
                      </div>
                    ...
                    {prodEvidence.hash && (
                      <div className="flex items-center gap-1 text-[10px] text-slate-500 font-mono px-1 bg-slate-900/60 p-1 rounded-lg border border-slate-800/80">
                        <Hash className="w-3 h-3 text-amber-500/70 shrink-0" />
                        <span className="text-amber-400/80 font-bold shrink-0">SHA-256:</span>
                        <span className="truncate">{prodEvidence.hash}</span>
                      </div>
                    )}
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-t-3xl sm:rounded-2xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-xs">
              ...
              {/* Environmental Reading Photographic Evidence */}
                ...
                {!envEvidence.name ? (
                  <label className="w-full p-2.5 bg-slate-950 border border-slate-800 hover:border-slate-700 rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all">
                    ...
                    <input
                      type="file"
                      accept="image/*,application/pdf"
                      onChange={e => handleEvidenceUpload(e, 'ENV')}
                      className="hidden"
                    />
                  </label>
                ) : (
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        {envEvidence.url && envEvidence.type?.startsWith('image/') ? (
                          <img
                            src={envEvidence.url}
                            alt="Evidence preview"
                            className="w-10 h-10 object-cover rounded-lg border border-slate-700 shrink-0"
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-slate-200 font-medium truncate">{envEvidence.name}</p>
                          {envEvidence.size && (
                            <p className="text-[10px] text-slate-500 font-mono">
                              {(envEvidence.size / 1024).toFixed(1)} KB
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {envEvidence.hash && (
                      <div className="flex items-center gap-1 text-[10px] text-slate-500 font-mono px-1 bg-slate-900/60 p-1 rounded-lg border border-slate-800/80">
                        <Hash className="w-3 h-3 text-amber-500/70 shrink-0" />
                        <span className="text-amber-400/80 font-bold shrink-0">SHA-256:</span>
                        <span className="truncate">{envEvidence.hash}</span>
                      </div>
                    )}
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-t-3xl sm:rounded-2xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-xs">
              ...
              {/* Compliance Non-Conformance Evidence */}
              <div className="space-y-1.5 pt-1">
                <label className="text-slate-400 block font-mono text-[11px] flex items-center justify-between">
                  <span>{t('evidenceAttachmentTitle')}</span>
                  <span className="text-[10px] text-slate-500 font-mono">SHA-256 Fingerprint</span>
                ...
                {!compEvidence.name ? (
                  <label className="w-full p-2.5 bg-slate-950 border border-slate-800 hover:border-slate-700 rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all">
                    ...
                    <input
                      type="file"
                      accept="image/*,application/pdf"
                      onChange={e => handleEvidenceUpload(e, 'COMP')}
                      className="hidden"
                    />
                  </label>
                ) : (
                  <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        {compEvidence.url && compEvidence.type?.startsWith('image/') ? (
                          <img
                            src={compEvidence.url}
                            alt="Evidence preview"
                            className="w-10 h-10 object-cover rounded-lg border border-slate-700 shrink-0"
                          />
                        ) : (
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-slate-200 font-medium truncate">{compEvidence.name}</p>
                          {compEvidence.size && (
                            <p className="text-[10px] text-slate-500 font-mono">
                              {(compEvidence.size / 1024).toFixed(1)} KB
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {compEvidence.hash && (
                      <div className="flex items-center gap-1 text-[10px] text-slate-500 font-mono px-1 bg-slate-900/60 p-1 rounded-lg border border-slate-800/80">
                        <Hash className="w-3 h-3 text-amber-500/70 shrink-0" />
                        <span className="text-amber-400/80 font-bold shrink-0">SHA-256:</span>
                        <span className="truncate">{compEvidence.hash}</span>
                      </div>
                    )}
                  </div>
                )}

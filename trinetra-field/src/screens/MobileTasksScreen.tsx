...
export const MobileTasksScreen: React.FC<MobileTasksScreenProps> = ({ onNavigateTab }) => {
  ...
  // Helper functions for SLA / deadline determination
  const getTaskDueDate = (task: GovernanceTask): Date | null => {
  ...
  const checkIsOverdue = (task: GovernanceTask, now: Date): boolean => {
    if (typeof task.is_overdue === 'boolean') {
      return task.is_overdue;
    }
    const status = (task.status || 'OPEN').toUpperCase();
    if (['RESOLVED', 'VERIFIED', 'CLOSED'].includes(status)) return false;
    const dueDate = getTaskDueDate(task);
    return dueDate ? dueDate < now : false;
  };

  const checkIsDueToday = (task: GovernanceTask, todayStr: string): boolean => {
    if (typeof task.is_due_today === 'boolean') {
      return task.is_due_today;
    }
    const status = (task.status || 'OPEN').toUpperCase();
    if (['RESOLVED', 'VERIFIED', 'CLOSED'].includes(status)) return false;
    const dateStr = task.due_at || task.due_date;
    return Boolean(dateStr && dateStr.startsWith(todayStr));
  };

  // Local fallback counts for real-time consistency
  const fallbackCounts = useMemo(() => {
    const now = new Date();
    const todayStr = now.toISOString().slice(0, 10);
    return {
      total: tasks.length,
      assigned: tasks.filter(t => (t.status || 'OPEN').toUpperCase() === 'ASSIGNED' || (t.status || 'OPEN').toUpperCase() === 'OPEN').length,
      in_progress: tasks.filter(t => (t.status || 'OPEN').toUpperCase() === 'IN_PROGRESS').length,
      due_today: tasks.filter(t => checkIsDueToday(t, todayStr)).length,
      overdue: tasks.filter(t => checkIsOverdue(t, now)).length,
      verification_pending: tasks.filter(t => (t.status || 'OPEN').toUpperCase() === 'RESOLVED' || (t.status || 'OPEN').toUpperCase() === 'SUBMITTED').length,
      completed: tasks.filter(t => ['COMPLETED', 'RESOLVED', 'VERIFIED', 'CLOSED'].includes((t.status || 'OPEN').toUpperCase())).length,
    };
  }, [tasks]);
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      ...
      let matchesTab = true;
      const status = (task.status || 'OPEN').toUpperCase();
      const isOverdue = checkIsOverdue(task, now);
      const isDueToday = checkIsDueToday(task, todayStr);
      ...
      if (activeTab === 'ASSIGNED') {
        ...
        matchesTab = isDueToday;
      } else if (activeTab === 'OVERDUE') {
        matchesTab = isOverdue;
      } else if (activeTab === 'COMPLETED') {
        matchesTab = status === 'COMPLETED' || status === 'RESOLVED' || status === 'VERIFIED' || status === 'CLOSED';
      } else if (activeTab === 'VERIFICATION') {
        matchesTab = status === 'RESOLVED' || status === 'SUBMITTED';
      }
      ...
      // Next by due date ascending (closest deadline first)
  ...
  const tabs: { key: TabType; label: string; count: number }[] = [
    { key: 'ALL', label: t('allTasks'), count: counts?.total ?? fallbackCounts.total },
    { key: 'ASSIGNED', label: 'ASSIGNED', count: counts?.assigned ?? fallbackCounts.assigned },
    { key: 'IN_PROGRESS', label: t('inProgress'), count: counts?.in_progress ?? fallbackCounts.in_progress },
    { key: 'DUE_TODAY', label: t('dueToday'), count: counts?.due_today ?? fallbackCounts.due_today },
    { key: 'OVERDUE', label: t('overdue'), count: counts?.overdue ?? fallbackCounts.overdue },
    { key: 'VERIFICATION', label: t('verificationQueue'), count: counts?.verification_pending ?? counts?.pending_verification ?? fallbackCounts.verification_pending },
    { key: 'COMPLETED', label: t('completed'), count: counts?.completed ?? fallbackCounts.completed },
  ];
  ...
  const getHeaderCountLabel = () => {
    const count = filteredTasks.length;
    const isFiltered = search.trim().length > 0 || priorityFilter !== 'ALL';
    
    if (isFiltered) {
      return `${count} ${count === 1 ? 'matching task' : 'matching tasks'}`;
    }
    
    switch (activeTab) {
      case 'ALL':
        return `${count} ${count === 1 ? 'total task' : 'total tasks'}`;
      case 'ASSIGNED':
        return `${count} ${count === 1 ? 'assigned task' : 'assigned tasks'}`;
      case 'IN_PROGRESS':
        return `${count} in progress`;
      case 'DUE_TODAY':
        return `${count} due today`;
      case 'OVERDUE':
        return `${count} ${count === 1 ? 'overdue task' : 'overdue tasks'}`;
      case 'VERIFICATION':
        return `${count} pending verification`;
      case 'COMPLETED':
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <span className="text-[11px] font-mono text-slate-400">
            {selectedMine?.name || 'Mine'} • {getHeaderCountLabel()}
          </span>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-8 text-center rounded-2xl bg-slate-900/50 border border-slate-800 text-slate-400 font-mono text-xs space-y-2">
          ...
          {(() => {
            ...
            if (isFiltered) {
              return (
                <>
                  <p className="font-semibold text-slate-300">No Matching Tasks Found</p>
                  <p className="text-[11px] text-slate-500">Try adjusting your search query or priority filter.</p>
                </>
              );
            }
            switch (activeTab) {
              case 'ASSIGNED':
                return (
                  <>
                    <p className="font-semibold text-slate-300">{t('noAssignedTasks')}</p>
                    <p className="text-[11px] text-slate-500">{t('noAssignedTasksDesc')}</p>
                  </>
                );
              case 'IN_PROGRESS':
                return (
                  <>
                    <p className="font-semibold text-slate-300">No Tasks In Progress</p>
                    <p className="text-[11px] text-slate-500">Start an assigned task to track ongoing work.</p>
                  </>
                );
              case 'DUE_TODAY':
                return (
                  <>
                    <p className="font-semibold text-slate-300">No Tasks Due Today</p>
                    <p className="text-[11px] text-slate-500">No statutory inspections or tasks scheduled for today.</p>
                  </>
                );
              case 'OVERDUE':
                return (
                  <>
                    <p className="font-semibold text-slate-300">{t('noOverdueTasks')}</p>
                    <p className="text-[11px] text-slate-500">All statutory tasks are currently within their SLA.</p>
                  </>
                );
              case 'VERIFICATION':
                return (
                  <>
                    <p className="font-semibold text-slate-300">{t('noPendingVerification')}</p>
                    <p className="text-[11px] text-slate-500">No completed tasks awaiting supervisor signoff.</p>
                  </>
                );
              case 'COMPLETED':
                return (
                  <>
                    <p className="font-semibold text-slate-300">No Completed Tasks</p>
                    <p className="text-[11px] text-slate-500">Resolved and closed tasks will appear here.</p>
                  </>
                );
              default:
                return (
        ...
        <div className="space-y-3">
          {filteredTasks.map((task) => {
            ...
            const isOverdue = checkIsOverdue(task, new Date());
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-mono text-slate-400 border-t border-slate-800/80 pt-2.5">
                  ...
                  <div className="flex items-center gap-1.5 justify-end">
                    ...
                    <span className={clsx(isOverdue ? 'text-red-400 font-bold' : 'text-slate-400')}>
                      {formatDueDate(task.due_at || task.due_date)}
                    </span>
                  </div>
                </div>

                {/* Assignment & Reason Snippet */}
                {(task.assignee_name || task.assigned_to_name) && (
            ...
            <div className="p-4 overflow-y-auto space-y-4 text-xs font-sans text-slate-300">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                ...
                <div className="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/80 space-y-1">
                  ...
                  <span className="font-semibold text-amber-400">{formatDueDate(selectedTask.due_at || selectedTask.due_date)}</span>
                </div>
              </div>
